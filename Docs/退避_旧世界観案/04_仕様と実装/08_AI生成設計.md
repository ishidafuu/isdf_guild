...
- 面談返答/日報/関係イベント文面では、人物の最近の経験を必ず参照する
- 追加入力キー:
- `experience_digest`: 直近重要イベント最大5件。各件は `day`, `event_kind`, `headline`, `emotion_digest`, `importance`
- `arc_tags`: 現在の人物傾向タグ。例: `慎重化`, `依頼主不信`, `名誉志向`, `復帰直後`, `相棒固定化`
- `big5_drift`: 現在の長期ドリフト値（`O/C/E/A/N` の `-12〜+12`）
- `career_eval_tags`: 非数値ラベル化した実務評価。例: `堅実`, `危なっかしい`, `華がある`, `伸び盛り`
- `volatile_hook` は上記と矛盾してはならない
- AIは人物の変化を「突然別人になる」のではなく、「最近こういう経験があったので今はこう反応する」として表現する

- 優先順位:
- 直近かつ `importance >= 60` の経験を最優先で反映する
- 同一テーマの経験が連続している場合は、それを現在の人物弧として扱う
- `private_dossier` は生来の癖、`experience_digest` は最近の揺れとして扱う
...
- 追加任意項目:
- `journal_headline`: 人物年表へ残す短い一文
- `emotion_phrase`: その場の受け止めを示す1フレーズ
- `lesson_tags`: `risk_suspicion`, `earned_confidence`, `guild_gratitude` などの学習タグ
- `journal_headline` は保存時の補助表示に使ってよいが、内部状態更新の正本はロジック層が持つ
